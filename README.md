...
| セット（大問） | 複数のカードを1つの大問としてまとめると `問3-(1)` `問3-(2)` のように枝番で採番されます |
| 使用しない問題 | 完成した問題を「使用しない」に切り替えると、採番・CSV出力・正解分布の集計から外れます（カードは残ります） |
...
- **問題番号**: 番号を保存せず、「完成」カラムの `order` から毎回計算しています。保存しないので、複数人が同時に並べ替えても番号が衝突しません。差し戻すと自動的に番号が外れ、後続の番号が繰り上がります。セットにまとめたカードは最初に現れた位置で大問番号が決まり、その中で `(1)(2)…` の枝番が付きます。
- **セット（大問）**: `CardGroup` に属するカード同士を1つの大問として扱います。カード詳細から相手を選ぶだけでセットになり、片方をセットから外して残りが1枚になるとセット自体が解散します。冊子では続けて出題される想定なので、ボード上でも隣に並べてください。
