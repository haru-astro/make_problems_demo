...
| メンバー切り替え | 認証は未実装のため、操作中のユーザーをCookieで切り替えます。メンバー追加も画面から可能 |
...
- **認証**: 未実装です。`src/lib/session.ts` の `getCurrentUser()` を認証基盤（NextAuth など）に差し替えれば、Server Actions 側の作成者・コメント投稿者の解決はそのまま使えます。
