...
| セット（大問） | 複数のカードを1つの大問としてまとめると、同じ問題番号（例: どちらも `問3`）になります。ドラッグやステータス変更では必ず一緒に移動します |
...
- **セット（大問）**: `CardGroup` に属するカード同士を1つの大問として扱います。カード詳細から相手を選ぶだけでセットになり、片方をセットから外して残りが1枚になるとセット自体が解散します。移動時は常にセット全員がまとめて動くため、冊子上でも隣り合った状態が保たれます。
